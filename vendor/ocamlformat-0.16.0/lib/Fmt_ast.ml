...
module Cmts = struct
  ...
  let fmt_list ?pro ?epi ?eol c locs init =
    List.fold locs ~init ~f:(fun k loc -> fmt ?pro ?epi ?eol ?adj:None c loc k)
end
...
let docstring_epi ~standalone ~next ~epi ~floating =
  ...
  match next with
  | (None | Some (_, false)) when floating && not standalone -> str "\n" $ epi
  | _ -> epi
  ...
  | None, Some doc | Some doc, None -> (
      ...
      let placement =
        if force_before then `Before
          ...
          Poly.( = ) c.conf.doc_comments_tag_only `Fit && fit && is_tag_only doc
        then `Fit
        else
      ...
      | `Fit ->
          (floating_doc, fmt_doc ~pro:(break c.conf.doc_comments_padding 0) doc)
      )
...
let fmt_quoted_string key ext s = function
  ...
  | Some delim ->
      let ext_and_delim =
        if String.is_empty delim then ext else Format.sprintf "%s %s" ext delim
      in
      wrap_k
...
and fmt_payload c ctx pld =
  ...
  | PSig mty ->
      str ":" $ fmt_if (not (List.is_empty mty)) "@ " $ fmt_signature c ctx mty
  | PTyp typ -> fmt ":@ " $ fmt_core_type c (sub_typ ~ctx typ)
  | PPat (pat, exp) ->
      let fmt_when exp = str " when " $ fmt_expression c (sub_exp ~ctx exp) in
...
and fmt_record_field c ?typ ?rhs ?(type_first = false) lid1 =
  ...
  $ cbox 0 (fmt_longident_loc c lid1 $ Cmts.fmt_after c lid1.loc $ fmt_type_rhs)
...
and fmt_core_type c ?(box = true) ?(in_type_declaration = false) ?pro
  ...
  | Ptyp_poly ([], _) -> impossible "produced by the parser, handled elsewhere"
  | Ptyp_poly (a1N, t) ->
      hovbox_if box 0
  ...
  | Ptyp_variant (rfs, flag, lbls) ->
      let row_fields rfs =
        ...
        | _ ->
            ...
            list rfs
              (if in_type_declaration && Poly.(c.conf.type_decl = `Sparse) then
               "@;<1000 0>| "
              else "@ | ")
  ...
  | Ptyp_object (fields, closedness) ->
      let fmt_field { pof_desc; pof_attributes = atrs; pof_loc } =
        ...
        Cmts.fmt c pof_loc
        @@ hvbox 4
             (hvbox 2 fmt_field $ fmt_docstring_padded c doc
             $ fmt_attributes c ~pre:Space ~key:"@" atrs)
      in
      hvbox 0
        (wrap "< " " >"
           (list fields "@ ; " fmt_field $ fmt_if (is_open closedness) "@ ; .."))
  ...
  | attrs ->
      let parens_attr =
        ...
        | _ -> (
            ...
            | _ -> true)
      in
      Params.parens_if parens_attr c.conf (k $ fmt_attributes c ~key:"@" attrs)
...
and fmt_pattern c ?pro ?parens ({ ctx = ctx0; ast = pat } as xpat) =
  ...
  | Ppat_tuple pats ->
      ...
      hvbox 0
        (Params.wrap_tuple ~parens ~no_parens_if_break:false c.conf
           (list pats (Params.comma_sep c.conf) (sub_pat ~ctx >> fmt_pattern c)))
      ...
      | Some (loc_xpats, nil_loc) ->
          ...
          hvbox 0
            (p.box
               (Cmts.fmt c ppat_loc
                  (pat
                  $ Cmts.fmt_before c ~pro:cmt_break ~epi:noop nil_loc ~eol:noop
  ...
  | Ppat_record (flds, closed_flag) ->
        ...
        hvbox 0
          ...
          | Ppat_constraint ({ ppat_desc = Ppat_var { txt; _ }; ppat_loc; _ }, t)
            when field_alias ~field:lid1.txt (Longident.lident txt)
                 && List.is_empty ppat_attributes ->
              if
                Ocaml_version.(compare Parse.parser_version Releases.v4_12) >= 0
              then
          ...
          | Ppat_constraint (p, t) when List.is_empty ppat_attributes ->
              ...
              let type_first = Source.type_constraint_is_first t p.ppat_loc in
      ...
      in
      hvbox_if parens 0
        (Params.parens_if parens c.conf (p1.box (fmt_fields $ fmt_underscore)))
  | Ppat_array [] ->
      hvbox 0 (wrap_fits_breaks c.conf "[|" "|]" (Cmts.fmt_within c ppat_loc))
  | Ppat_array pats ->
        ...
        (list_fl (List.group xpats ~break)
           (fun ~first:first_grp ~last:_ xpat_grp ->
             list_fl xpat_grp (fun ~first ~last xpat ->
                 ...
                 in
                 leading_cmt $ fmt_pattern c ~pro xpat $ fmt_if_k last close_box))
        $ fmt_or_k nested
        ...
        (Params.parens_if parens c.conf
           (hvbox 1
              (Cmts.fmt c typ.ptyp_loc
                 (hovbox 0
                    (Cmts.fmt c ppat_loc (str "module " $ fmt_str_loc_opt c name)
  ...
  let fmt_fun_arg (a : Sugar.arg_kind) =
    ...
    | Val (Optional l, xpat, Some xexp) ->
        ...
        cbox 2
          (str "?" $ str l
          $ wrap_k (fmt ":@,(") (str ")")
              (fmt_pattern c ?parens xpat $ fmt " =@;<1 2>"
    ...
    | Newtypes names ->
        cbox 0
          (Params.parens c.conf (str "type " $ list names "@ " (fmt_str_loc c)))
  in
  fmt_if_k (not (List.is_empty args)) (fmt_opt pro $ list args "@;" fmt_fun_arg)
...
and fmt_body c ?ext ({ ast = body; _ } as xbody) =
  ...
  | { pexp_desc = Pexp_function cs; pexp_attributes; pexp_loc; _ } ->
      ( ( update_config_maybe_disabled c pexp_loc pexp_attributes @@ fun c ->
          fmt "@ " $ Cmts.fmt_before c pexp_loc $ fmt_if parens "("
          $ str "function" $ fmt_extension_suffix c ext
...
and fmt_index_op c ctx ~fmt_atrs ~has_attr ~parens op =
  ...
  let fmt_args, fmt_op, brackets =
    ...
    | Extended (args, cop) ->
        (list args (Params.semi_sep c.conf) fmt_arg, fmt_cop cop, cop.brackets)
...
and fmt_sequence c ?ext ~has_attr parens width xexp pexp_loc fmt_atrs =
  let fmt_sep c ?(force_break = false) xe1 ext xe2 =
    ...
    | `Before ->
        break $ str ";" $ fmt_extension_suffix c ext
        $ fmt_or_k (Option.is_some ext) (fmt_or parens "@ " "@;<1 2>") (str " ")
  ...
  let break (_, xexp1) (_, xexp2) = not (is_simple xexp1 && is_simple xexp2) in
...
and fmt_expression c ?(box = true) ?pro ?epi ?eol ?parens ?(indent_wrap = 0)
           ...
           (hvbox 2
              (hvbox indent_wrap
                 ...
                 $ hovbox 2
                     ...
                     $ hvbox_if (not c.conf.wrap_fun_args) 4
            ...
            (Params.parens_if parens c.conf
               (hovbox 4
                  (wrap
                    ...
                    $ Cmts.fmt_before c pexp_loc $ fmt_label lbl ":"
                    $ str "(function"
                     $ fmt_attributes c ~pre:Blank ~key:"@" eN.pexp_attributes)
                  $ fmt "@ " $ leading_cmt
                  $ hvbox 0
  ...
  | Pexp_construct ({ txt = Lident (("()" | "[]") as txt); loc }, None) ->
      ...
      @@ hvbox 0
           (Params.parens_if parens c.conf
              (wrap_k opn cls (Cmts.fmt_within c ~pro ~epi pexp_loc) $ fmt_atrs))
  ...
  | Pexp_newtype _ | Pexp_fun _ ->
      ...
      let indent = Params.function_indent c.conf ~ctx ~default:default_indent in
  ...
  | Pexp_function cs ->
      let indent = Params.function_indent c.conf ~ctx in
      Params.wrap_exp c.conf c.source ~loc:pexp_loc ~parens ~disambiguate:true
        ~fits_breaks:false
        (hvbox 2
           (str "function" $ fmt_extension_suffix c ext
           $ fmt_attributes c ~key:"@" pexp_attributes)
  ...
  | Pexp_ident { txt; loc } ->
      ...
      in
      Cmts.fmt c loc
      @@ wrap (wrap_ident (fmt_longident txt $ Cmts.fmt_within c loc) $ fmt_atrs)
  | Pexp_ifthenelse _ ->
      let cnd_exps = Sugar.ite c.cmts xexp in
      let parens_prev_bch = ref false in
      hvbox 0
        (Params.wrap_exp c.conf c.source ~loc:pexp_loc ~parens
           (list_fl cnd_exps (fun ~first ~last (xcond, xbch, pexp_attributes) ->
  ...
  | Pexp_let (rec_flag, bindings, body) ->
      ...
      fmt_let c ctx ~ext ~rec_flag ~bindings ~parens ~loc:pexp_loc
        ~attributes:pexp_attributes ~fmt_atrs ~fmt_expr ~body_loc:body.pexp_loc
        ~indent_after_in
  | Pexp_letexception (ext_cstr, exp) ->
      let pre = str "let exception" $ fmt_extension_suffix c ext $ fmt "@ " in
      hvbox 0
        (Params.parens_if
           (parens || not (List.is_empty pexp_attributes))
           c.conf
           (hvbox 0
              (hvbox 2
                 (hvbox 2
                    (pre $ fmt_extension_constructor c (str ": ") ctx ext_cstr))
              $ fmt "@ in")
           $ fmt "@;<1000 0>"
           $ fmt_expression c (sub_exp ~ctx exp))
        $ fmt_atrs)
  ...
  | Pexp_pack me ->
      let fmt_mod m =
        Params.parens_if parens c.conf
          ...
          $ fmt_atrs)
      in
      hvbox 0 (compose_module (fmt_module_expr c (sub_mod ~ctx me)) ~f:fmt_mod)
      ...
      let fmt_field (lid1, f) =
        ...
        hvbox 0
          ...
          | Pexp_constraint (e, t) when List.is_empty f.pexp_attributes ->
              let type_first = Source.type_constraint_is_first t e.pexp_loc in
  ...
  | Pexp_tuple es ->
      ...
      hvbox_if outer_wrap 0
        (Params.parens_if outer_wrap c.conf
           (hvbox 0
              (Params.wrap_tuple ~parens:inner_wrap ~no_parens_if_break c.conf
                 ...
                 Pstr_eval
                   ( ({
                        pexp_desc =
                          ...
                          | Pexp_function _ | Pexp_letexception _ | Pexp_open _
                            );
                        pexp_attributes = [];
                        _;
                      } as e1),
      ...
      hvbox 0
        (Params.wrap_exp c.conf c.source ~loc:pexp_loc ~parens
           (hovbox 0
              (hvbox 2
                 (hvbox 0
                    (str "for" $ fmt_extension_suffix c ext $ fmt "@;<1 2>"
                    $ hovbox 0
      ...
      hvbox 0
        (Params.wrap_exp c.conf c.source ~loc:pexp_loc ~parens
           (hovbox 0
              (hvbox 2
                 (hvbox 0
                    (str "while" $ fmt_extension_suffix c ext $ fmt "@;<1 2>"
                    $ fmt_expression c (sub_exp ~ctx e1)
                    $ fmt "@;do")
                 $ fmt "@;<1000 0>"
                 $ fmt_expression c (sub_exp ~ctx e2))
  ...
  | Pexp_new { txt; loc } ->
      ...
      @@ hvbox 2
           (Params.parens_if parens c.conf
              (str "new" $ fmt_extension_suffix c ext $ fmt "@ "
             $ fmt_longident txt $ fmt_atrs))
  | Pexp_object { pcstr_self; pcstr_fields } ->
      hvbox 0
        (Params.parens_if parens c.conf
           (fmt_class_structure c ~ctx ?ext pcstr_self pcstr_fields $ fmt_atrs))
  | Pexp_override l -> (
...
and fmt_class_structure c ~ctx ?ext self_ fields =
  ...
  hvbox 2
    (hvbox 0
       (str "object" $ fmt_extension_suffix c ext
       $ opt self_ (fun self_ ->
             fmt "@;"
...
and fmt_class_expr c ?eol ?(box = true) ({ ast = exp; _ } as xexp) =
  ...
  | Pcl_let (rec_flag, bindings, body) ->
      let indent_after_in =
        match body.pcl_desc with Pcl_let _ -> 0 | _ -> c.conf.indent_after_in
      in
...
and fmt_class_field c ctx (cf : class_field) =
  ...
  let fmt_kind = function
    ...
    | Cfk_concrete
        ...
        | Some (args, t, e) ->
            let before = match args with x :: _ -> x.loc | [] -> e.pexp_loc in
    ...
    | Cfk_concrete (_, { pexp_desc = Pexp_poly (e, poly); pexp_loc; _ }) ->
        let xargs, xbody =
          match poly with
          | None ->
              Sugar.fun_ c.cmts ~will_keep_first_ast_node:false (sub_exp ~ctx e)
...
and fmt_case c ctx ~first ~last ~padding case =
  ...
  in
  let eol =
    Option.some_if (Cmts.has_before c.cmts pc_rhs.pexp_loc) (fmt "@;<1000 0>")
  in
...
and fmt_value_description ?ext c ctx vd =
  ...
  hvbox 0
    (doc_before
    $ box_fun_sig_args c 2
        (str pre $ fmt_extension_suffix c ext $ str " "
        $ Cmts.fmt c loc (wrap_if (String_id.is_symbol txt) "( " " )" (str txt))
        $ fmt_core_type c ~pro:":"
            ~box:(not (c.conf.ocp_indent_compat && is_arrow_or_poly pval_type))
...
and fmt_type_declaration c ?ext ?(pre = "") ctx ?fmt_name ?(eq = "=") decl =
  ...
  let fmt_manifest_kind =
    ...
    | Ptype_variant [] ->
        box_manifest (fmt_manifest ~priv:Public (Some (fmt_private_flag priv)))
        $ fmt "@ |"
    | Ptype_variant ctor_decls ->
        ...
        box_manifest (fmt_manifest ~priv:Public (Some (fmt_private_flag priv)))
        $ fmt "@ "
        $ list_fl ctor_decls (fmt_constructor_declaration c ~max_len_name ctx)
    ...
    | Ptype_open ->
        box_manifest
          (fmt_manifest ~priv:Public (Some (fmt_private_flag priv $ str " ..")))
  in
        ...
        (hvbox 3
           (hvbox 4
              (hvbox 2
                 ...
                 $ fmt_str_loc c pld_name $ fmt_if field_loose " " $ fmt ":@ "
                 $ fmt_core_type c (sub_typ ~ctx pld_type)
                 $ fmt_semicolon)
              $ cmt_after_type)
           $ fmt_attributes c ~pre:(Break (1, 1)) ~key:"@" atrs)
        $ fmt_docstring_padded c doc $ Cmts.fmt_after c pld_loc))
...
and fmt_constructor_declaration c ctx ~max_len_name ~first ~last:_ cstr_decl =
  ...
  let fmt_padding =
    ...
    let pad = String.make (max_len_name - String.length txt - len_around) ' ' in
    fmt_if_k
...
and fmt_type_extension ?ext c ctx
  ...
  @@ hvbox 2
       ...
       $ hvbox c.conf.type_decl_indent
           (str "type" $ fmt_extension_suffix c ext $ str " "
           $ hvbox_if
...
and fmt_type_exception ~pre c sep ctx
  ...
  Cmts.fmt c ptyexn_loc
    (hvbox 0
       (doc_before
       $ hvbox 2 (pre $ fmt_extension_constructor c sep ctx ptyexn_constructor)
       $ fmt_attributes c ~pre:(Break (1, 0)) ~key:"@@" atrs
       $ doc_after))
...
and fmt_module_type c ({ ast = mty; _ } as xmty) =
  ...
  | Pmty_ident lid ->
      {
        ...
        epi =
          Some (fmt_attributes c ~key:"@" pmty_attributes ~pre:(Break (1, 0)));
      }
  | Pmty_signature s ->
  ...
  | Pmty_with _ ->
      ...
      in
      {
        empty with
        pro =
          Option.map pro ~f:(fun pro -> open_hvbox 0 $ fmt_if parens "(" $ pro);
        psp;
        bdy =
          fmt_if_k (Option.is_none pro) (open_hvbox 2 $ fmt_if parens "(")
          $ hvbox 0 bdy
          $ fmt_if_k (Option.is_some epi) esp
          $ fmt_opt epi $ list_fl wcs fmt_cstrs $ fmt_if parens ")" $ close_box;
      ...
      | Some pro ->
          {
            ...
            pro =
              Some
                (Cmts.fmt_before c pmty_loc $ fmt_if parens "("
      ...
      | _ ->
          {
            ...
            bdy =
              Cmts.fmt c pmty_loc
              @@ hvbox 2 (fmt_if parens "(" $ fmt "module type of@ " $ blk.bdy);
            epi = Some epi;
          })
  | Pmty_extension ext ->
      {
...
and fmt_class_types ?ext c ctx ~pre ~sep (cls : class_type class_infos list) =
  list_fl cls (fun ~first ~last:_ cl ->
      ...
      in
      fmt_if (not first) "\n@;<1000 0>"
      $ hovbox 0 @@ Cmts.fmt c cl.pci_loc (doc_before $ class_types $ doc_after))
  ...
  hvbox
    ...
    $ box_b
        ((if Option.is_some blk_t.epi then hovbox else hvbox)
           0
           (box_t
              (hvbox_if (Option.is_some blk_t.pro) 0
                 ((match arg_blks with
...
and fmt_module_expr ?(can_break_before_struct = false) c
      ...
      if Option.is_some blk_a.pro then
        {
          ...
          pro =
            Some
              (Cmts.fmt_before c pmod_loc $ hvbox 2 fmt_rator
             $ fmt_opt blk_a.pro);
          epi = Some epi;
        }
      else
  ...
  | Pmod_structure sis ->
      let empty = List.is_empty sis && not (Cmts.has_within c.cmts pmod_loc) in
      let doc, atrs = doc_atrs pmod_attributes in
  ...
  | Pmod_unpack
      ...
      in
      {
        empty with
        pro =
          Some (Cmts.fmt_before c pmod_loc $ fmt_docstring c ~epi:(fmt "@,") doc);
        bdy =
          Cmts.fmt c pmod_loc
          @@ hovbox 0
               (wrap_fits_breaks ~space:false c.conf "(" ")"
...
and fmt_structure_item c ~last:last_item ?ext { ctx; ast = si } =
  ...
  | Pstr_exception extn_constr ->
      ...
      hvbox 2 ~name:"exn" (fmt_type_exception ~pre c (str ": ") ctx extn_constr)
    ...
    ->
      ...
      let keyword =
        fmt_or_k
          ...
          (str "open!" $ opt ext (fun _ -> str " " $ fmt_extension_suffix c ext))
...
and fmt_let c ctx ~ext ~rec_flag ~bindings ~parens ~fmt_atrs ~fmt_expr ~loc
  ...
  let fmt_binding ~first ~last binding =
    ...
    fmt_value_binding c op ~rec_flag ?ext ctx ~in_ ~attributes ~loc pvb_pat
      pvb_expr
    $ fmt_if (not last)
        (match c.conf.let_and with `Sparse -> "@;<1000 0>" | `Compact -> "@ ")
  in
  ...
  $ hvbox indent
      (hovbox 2
         (hovbox 4
            (box_fun_decl_args c 4
               (hovbox 4
                  (str let_op $ fmt_extension_suffix c ext
...
let fmt_toplevel_directive c dir =
  ...
  let args =
    ...
    | Some { pdira_desc; pdira_loc; _ } ->
        ...
        $ fmt_dir_arg pdira_desc $ Cmts.fmt_after c pdira_loc
  in

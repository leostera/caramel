...
let wrap_exp (c : Conf.t) ?(disambiguate = false) ?(fits_breaks = true) ~parens
    ~loc source k =
...
let get_or_pattern_sep ?(cmts_before = false) ?(space = false) (c : Conf.t) ~ctx
    =

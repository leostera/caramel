    ...
    [
      ...
      `P
        "Unless otherwise noted, any option $(b,--)$(i,option)$(b,=)$(i,VAL) \
         ...
         default value is used if no other value is specified. The value of a \
         boolean option $(b,--foo) or $(b,--no-foo) can be modified in an \
         $(b,.ocamlformat) configuration file with '$(b,foo = \
         ){$(b,true),$(b,false)}', it can be done for any other option with an \
         '$(b,option = )$(i,VAL)' line (*), or using the OCAMLFORMAT \
         environment variable: \
         $(b,OCAMLFORMAT=)$(i,option)$(b,=)$(i,VAL)$(b,,)...$(b,,)$(i,option)$(b,=)$(i,VAL), \
         or as an optional parameter on the command line, or with a global \
         $(b,[@@@ocamlformat \")$(i,option)$(b,=)$(i,VAL)$(b,\"]) attribute in \
         the processed file, or with an $(b,[@@ocamlformat \
         \")$(i,option)$(b,=)$(i,VAL)$(b,\"]) attribute on expression in the \
         processed file.";
      `P
        "(*) $(b,.ocamlformat) files in current and all ancestor directories \
         for each input file are used, as well as the global $(b,ocamlformat) \
         file defined in $(b,\\$XDG_CONFIG_HOME) or in $(b,\\$HOME/.config) if \
         $(b,\\$XDG_CONFIG_HOME) is undefined. The global $(b,ocamlformat) \
         file has the lowest priority, then the closer the directory is to the \
         processed file, the higher the priority. The global $(b,ocamlformat) \
         file is only used when the option \
         $(b,enable-outside-detected-project) is set.";
      `P
        "If the $(b,disable) option is not set, an $(b,.ocamlformat-ignore) \
         file specifies files that OCamlFormat should ignore. Each line in an \
         $(b,.ocamlformat-ignore) file specifies a filename relative to the \
         directory containing the $(b,.ocamlformat-ignore) file. Shell-style \
      ...
      `P
        "If the $(b,disable) option is set, an $(b,.ocamlformat-enable) file \
         ...
         $(b,disable) option is set. Each line in an $(b,.ocamlformat-enable) \
         file specifies a filename relative to the directory containing the \
         $(b,.ocamlformat-enable) file. Shell-style regular expressions are \
         supported. Lines starting with $(b,#) are ignored and can be used as \
         comments.";
    ]
  in
...
module Formatting = struct
  ...
  let assignment_operator =
    ...
    let all =
      [
        ( "end-line",
          `End_line,
          "$(b,end-line) positions assignment operators (`:=` and `<-`) at the \
           end of the line and breaks after it if the whole assignment \
  ...
  let break_before_in =
    let doc =
      "Whether the line should break before the $(i,in) keyword of a $(i,let) \
       binding."
    in
    ...
    let all =
      [
        ( "fit-or-vertical",
          `Fit_or_vertical,
          "$(b,fit-or-vertical) will always break the line before the $(i,in) \
           keyword if the whole $(i,let) binding does not fit on a single \
           line." );
        ( "auto",
          `Auto,
          "$(b,auto) will only break the line if the $(i,in) keyword does not \
  ...
  let break_cases =
    ...
    let all =
      [
        ...
        ( "toplevel",
          `Toplevel,
          "$(b,toplevel) forces top-level cases (i.e. not nested or-patterns) \
           to break across lines, otherwise break naturally at the margin." );
        ( "fit-or-vertical",
          `Fit_or_vertical,
          "$(b,fit-or-vertical) tries to fit all or-patterns on the same line, \
           otherwise breaks." );
        ( "all",
          `All,
  ...
  let break_collection_expressions =
    ...
    let all =
      [
        ...
        ( "wrap",
          `Wrap,
          "$(b,wrap) will group simple expressions and try to format them in a \
           single line." );
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let break_fun_decl =
    ...
    let all =
      [
        ...
        ( "fit-or-vertical",
          `Fit_or_vertical,
          "$(b,fit-or-vertical) vertically breaks arguments if they do not fit \
           on a single line." );
        ( "smart",
          `Smart,
          "$(b,smart) is like $(b,fit-or-vertical) but try to fit arguments on \
           their line if they fit." );
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let break_sequences =
    let doc = "Force sequence expressions to break irrespective of margin." in
  ...
  let break_struct =
    ...
    let all =
      [
        ...
        ( "natural",
          `Natural,
          "$(b,natural) will break struct-end phrases naturally at the margin."
        );
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let doc_comments =
    ...
    let all =
      [
        ( "after-when-possible",
          `After_when_possible,
          "$(b,after-when-possible) puts doc comments after the corresponding \
           code. This option has no effect on variant declarations because \
           that would change their meaning and on structures, signatures and \
      ...
      ~removed_values:
        [
          C.removed_value ~name:"after" ~version:"0.14.2"
            ~msg:
              "This value has been renamed `after-when-possible` to take into \
               account the technical limitations of ocamlformat, the behavior \
               is unchanged.";
        ]
  ...
  let doc_comments_padding =
    let docv = "PADDING" in
    let doc = "Add $(docv) spaces before doc comments in type declarations." in
  ...
  let ( (* doc_comments_val *) ) =
    ...
    let msg =
      "If you are using `doc-comments-val=before` in combination with \
       ...
       to achive the same behavior. If you are using `doc-comments-val=before` \
       in combination with `doc-comments=after` this behavior is not available \
       anymore. If you are using `doc-comments-val=after` in combination with \
       `doc-comments=before` please now use `doc-comments=before-except-val`. \
       If you are using `doc-comments-val=after` in combination with \
       `doc-comments=after` then only `doc-comments=after-when-possible` is \
       now required to achieve the same behavior. If you are using \
  ...
  let dock_collection_brackets =
    let doc =
      "Dock the brackets of lists, arrays and records, so that when the \
       collection does not fit on a single line the brackets are opened on the \
       preceding line and closed on the following line."
    in
  ...
  let exp_grouping =
    ...
    let all =
      [
        ("parens", `Parens, "$(b,parens) groups expressions using parentheses.");
        ( "preserve",
          `Preserve,
  ...
  let function_indent_nested =
    let doc =
      "Whether the $(b,function-indent) parameter should be applied even when \
       in a sub-block."
    in
    ...
    let all =
      [
        ( "never",
          `Never,
          "$(b,never) only applies $(b,function-indent) if the function block \
           starts a line." );
        ("always", `Always, "$(b,always) always apply $(b,function-indent).");
        ("auto", `Auto, "$(b,auto) applies $(b,function-indent) when seen fit.");
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let if_then_else =
    ...
    let all =
      [
        ( "compact",
          `Compact,
          "$(b,compact) tries to format an if-then-else expression on a single \
           line." );
        ( "fit-or-vertical",
          `Fit_or_vertical,
          "$(b,fit-or-vertical) vertically breaks branches if they do not fit \
           on a single line." );
        ( "keyword-first",
          `Keyword_first,
  ...
  let indicate_nested_or_patterns =
    let doc =
      "Control whether or not to indicate nested or-pattern using indentation."
    in
    ...
    let all =
      [
        ( "unsafe-no",
          `Unsafe_no,
          "$(b,unsafe-no) does not indicate nested or-patterns. Warning: this \
           can produce confusing code where a short body of a match case is \
  ...
  let infix_precedence =
    ...
    let all =
      [
        ( "indent",
          `Indent,
          "$(b,indent) uses indentation to explicitly disambiguate precedences \
           of infix operators." );
        ( "parens",
          `Parens,
          "$(b,parens) uses parentheses to explicitly disambiguate precedences \
           of infix operators." );
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let let_binding_indent =
    let docv = "COLS" in
    let doc =
      "Indentation of let binding expressions ($(docv) columns) if they do not \
       fit on a single line."
    in
  ...
  let let_binding_spacing =
    ...
    let all =
      [
        ...
        ( "sparse",
          `Sparse,
          "$(b,sparse) places two open lines between a multi-line module-level \
           let binding and the next." );
        ( "double-semicolon",
          `Double_semicolon,
      ...
      [
        ( "compact",
          ...
          "$(b,compact) does not break a line after the $(i,let module ... =) \
           and before the $(i,in) if the module declaration does not fit on a \
           single line." );
        ( "sparse",
          `Sparse,
  ...
  let match_indent_nested =
    let doc =
      "Whether the $(b,match-indent) parameter should be applied even when in \
       a sub-block."
    in
    ...
    let all =
      [
        ( "never",
          `Never,
          "$(b,never) only applies $(b,match-indent) if the match block starts \
           a line." );
  ...
  let max_indent =
    let docv = "COLS" in
    let doc =
      "Maximum offset ($(docv) columns) added to a new line in addition to the \
       offset of the previous line."
    in
    C.any
  ...
  let nested_match =
    ...
    let all =
      [
        ...
        ( "align",
          `Align,
          "$(b,align) vertically aligns the nested pattern-matching under the \
           encompassing pattern-matching." );
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let parens_tuple =
    ...
    let all =
      [
        ("always", `Always, "$(b,always) always uses parentheses around tuples.");
        ( "multi-line-only",
          `Multi_line_only,
  ...
  let sequence_blank_line =
    ...
    let all =
      [
        ...
        ( "compact",
          `Compact,
          "$(b,compact) will not keep any blank line between expressions of a \
           sequence." );
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let sequence_style =
    ...
    let all =
      [
        ...
        ("before", `Before, "$(b,before) breaks the sequence before semicolons.");
      ]
    in
    C.choice ~names ~all ~doc ~section
  ...
  let stritem_extension_indent =
    let docv = "COLS" in
    let doc =
      "Indentation of structure items inside extension nodes ($(docv) columns)."
    in
  ...
  let type_decl =
    ...
    let all =
      [
        ( "compact",
          `Compact,
          "$(b,compact) will try to format constructors and records definition \
           in a single line." );
        ( "sparse",
          `Sparse,
  ...
  let wrap_comments =
    let doc =
      "Wrap comments and docstrings. Comments and docstrings are divided into \
       paragraphs by open lines (two or more consecutive newlines), and each \
       paragraph is wrapped at the margin. Multi-line comments with \
...
let comment_check =
  let default = true in
  let doc =
    "Control whether to check comments and documentation comments. Unsafe to \
     turn off. May be set in $(b,.ocamlformat)."
  in
...
let config =
  let doc =
    "Aggregate options. Options are specified as a comma-separated list of \
     pairs: $(i,option)$(b,=)$(i,VAL)$(b,,)...$(b,,)$(i,option)$(b,=)$(i,VAL)."
  in
...
let name =
  ...
  let doc =
    "Name of input file for use in error reporting and starting point when \
     searching for '.ocamlformat' files. Defaults to the input file name. Some \
     options can be specified in configuration files named '.ocamlformat' in \
     the same or a parent directory of $(docv), see documentation of other \
     options for details."
  in
  let default = None in
  mk ~default
...
let format_invalid_files : [ `Never | `Auto ] option ref =
  let doc =
    "How invalid (unparsable) files are formatted. $(b,never) doesn't format \
     invalid files and the parsing error will be printed. $(b,auto) will print \
     invalid parts of the input as verbatim text. The default value is \
     $(b,never). This option is experimental."
  in
...
let print_config =
  let doc =
    "Print the configuration determined by the environment variable, the \
     configuration files, preset profiles and command line. Attributes are not \
     considered. If many input files are specified, only print the \
     configuration for the first file. If no input file is specified, print \
     the configuration for the root directory if specified, or for the current \
...
let no_version_check =
  let doc =
    "Do not check that the version matches the one specified in .ocamlformat."
  in
  let default = false in
  ...
  let all =
    [
      ...
      ( "compact",
        ...
        "The $(b,compact) profile is similar to $(b,ocamlformat) but opts for \
         a generally more compact code style." );
      ( "sparse",
        Some sparse_profile,
        "The $(b,sparse) profile is similar to $(b,ocamlformat) but opts for a \
         generally more sparse code style." );
      ( "ocamlformat",
        Some ocamlformat_profile,
        "The $(b,ocamlformat) profile aims to take advantage of the strengths \
         of a parsetree-based auto-formatter, and to limit the consequences of \
         the weaknesses imposed by the current implementation. This is a style \
         which optimizes for what the formatter can do best, rather than to \
         match the style of any existing code. General guidelines that have \
         directed the design include: Legibility, in the sense of making it as \
         hard as possible for quick visual parsing to give the wrong \
         interpretation, is of highest priority; Whenever possible the \
         high-level structure of the code should be obvious by looking only at \
         the left margin, in particular, it should not be necessary to \
         ...
         without scrolling is easier, so indentation or white space is avoided \
         unless it helps legibility; Attention has been given to making some \
         syntactic gotchas visually obvious." );
      ( "janestreet",
  ...
  let update ~config ~from ~name ~value =
    ...
    | "version", `File _ ->
        if String.equal Version.version value || !no_version_check then
          ...
          Error
            (`Bad_value
              ( name,
                Format.sprintf "expecting %S but got %S" Version.version value
              ))
    | name, `File x ->
    ...
    | name, `Attribute ->
        if !disable_conf_attrs then (
          ...
          C.update ~config ~from:(`Parsed `Attribute) ~name ~value ~inline:true
  in
                ...
                | Ok file_on_current_line -> (
                    ...
                    if Fpath.equal filename f then Some (listing_file, lno)
                      ...
                      try
                        ...
                        in
                        Option.some_if (Re.execp re filename) (listing_file, lno)
...
let validate_inputs () =
  ...
  | _ :: _ :: _, _, Some _ -> Error "Cannot specify --name with multiple inputs"
  | (_ :: _ :: _ as inputs), None, None ->
      List.map inputs ~f:(function
        | Stdin -> Error "Cannot specify stdin together with other inputs"
        | File f ->
            let kind = Option.value ~default:(Kind Use_file) (kind_of_ext f) in
            Ok (kind, f))
      |> Result.all
...
let make_action ~enable_outside_detected_project ~root action inputs =
  ...
  | (`No_action | `Output _), `Several_files _ ->
      Error "Must specify exactly one input file without --inplace or --check"
  | `Inplace, `Stdin _ -> Error "Cannot specify stdin together with --inplace"
  | `No_action, `Single_file (kind, name, f) ->
      Ok (In_out (make_file ?name kind f, None))
  | `No_action, `Stdin (name, kind) -> Ok (In_out (make_stdin ?name kind, None))
...
let validate () =
  ...
  if !disable_outside_detected_project then
    warn
      "option `--disable-outside-detected-project` is deprecated and will be \
       removed in OCamlFormat v1.0.";
  match
    let open Result in

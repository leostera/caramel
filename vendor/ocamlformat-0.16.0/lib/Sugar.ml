  ...
  let rec infix_ ?(relocate = true) xop ((lbl, { ast = exp; _ }) as xexp) =
    ...
    | Left, { pexp_desc = Pexp_apply (e0, [ (l1, e1); (l2, e2) ]); pexp_loc; _ }
      when Option.equal Prec.equal prec (prec_ast (Exp exp)) ->
        ...
        | _ ->
            if relocate then Cmts.relocate cmts ~src ~before:e0.pexp_loc ~after);
        op_args1 @ [ (Some (sub_exp ~ctx e0), [ (l2, sub_exp ~ctx e2) ]) ]
    | ( Right,
        { pexp_desc = Pexp_apply (e0, [ (l1, e1); (l2, e2) ]); pexp_loc; _ } )
      when Option.equal Prec.equal prec (prec_ast (Exp exp)) ->
        let op_args2 = infix_ (Some (sub_exp ~ctx e0)) (l2, sub_exp ~ctx e2) in
        let src = pexp_loc in
        let after = e1.pexp_loc in
        let before =
          match xop with Some { ast; _ } -> ast.pexp_loc | None -> e1.pexp_loc
        in
...
let rec ite cmts ({ ast = exp; _ } as xexp) =
  ...
  | Pexp_ifthenelse (cnd, thn, Some els) ->
      Cmts.relocate cmts ~src:pexp_loc ~before:cnd.pexp_loc ~after:els.pexp_loc;
      (Some (sub_exp ~ctx cnd), sub_exp ~ctx thn, pexp_attributes)
      :: ite cmts (sub_exp ~ctx els)
  | Pexp_ifthenelse (cnd, thn, None) ->
      Cmts.relocate cmts ~src:pexp_loc ~before:cnd.pexp_loc ~after:thn.pexp_loc;
      [ (Some (sub_exp ~ctx cnd), sub_exp ~ctx thn, pexp_attributes) ]
  ...
  let rec sequence_ ?(allow_attribute = true) ({ ast = exp; _ } as xexp) =
    ...
    | Pexp_extension
      ...
      when List.is_empty pexp_attributes
        ...
        let ctx = Exp exp in
        Cmts.relocate cmts ~src:pexp_loc ~before:e1.pexp_loc ~after:e2.pexp_loc;
        if (not allow_attribute) && not (List.is_empty exp.pexp_attributes) then
          [ (None, xexp) ]

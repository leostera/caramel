...
let str_normalized ?(escape = escape_all) s =
  split_on_whitespaces s |> List.filter ~f:(Fn.non String.is_empty) |> fun s ->
  list s "@ " (fun s -> escape s |> str)
...
let fmt_block_element c = function
  ...
  | `Heading (lvl, lbl, elems) ->
      ...
      let lbl =
        ...
        | None -> fmt ""
      in
      let elems = if List.is_empty elems then elems else space_elt :: elems in

  ...
  -spec start(A) -> beam__erlang:pid(B).
  ...
  -spec a_loop(A, fun((beam__process:after_time()) -> option:t(integer())), integer()) -> B.
  a_loop(Pid, Recv, I) ->
  ...
  -spec a_loop(A, fun((beam__process:after_time()) -> option:t({call, {beam__erlang:pid(integer()), integer()}}
      | noop
      )), integer()) -> B.
  a_loop(Pid, Recv, I) ->
    case Recv(infinity) of

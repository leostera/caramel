...
let rec odoc_inline_element fmt = function
  ...
  | `Word txt ->
      ...
      let txt = String.filter txt ~f:(function '\\' -> false | _ -> true) in
...
let odoc_tag c fmt = function
  ...
  | `Return elems -> fpf fmt "Return,%a" (odoc_nestable_block_elements c) elems
  | `See (kind, txt, elems) ->
      let kind =
        match kind with `Url -> "U" | `File -> "F" | `Document -> "D"
      in
      fpf fmt "See,%s,%a,%a" kind str txt (odoc_nestable_block_elements c) elems
...
let sort_attributes : attributes -> attributes = List.sort ~compare:Poly.compare
...
let make_mapper conf ~ignore_doc_comments =
  ...
  object (self)
                       ...
                       pstr_desc =
                         Pstr_eval
                           ( {
                               ...
                               pexp_attributes = self#attributes pexp_attributes;
                               pexp_loc_stack = [];
                             },
                             [] );
    ...
    method! value_binding vb =
      let {
        pvb_pat = { ppat_desc; ppat_loc; ppat_attributes; _ };
        pvb_expr;
        pvb_loc;
        pvb_attributes;
      } =
      ...
      | Ppat_constraint (p0, t0), _ ->
          self#value_binding
            (Vb.mk ~loc:pvb_loc ~attrs:pvb_attributes p0
               (Exp.constraint_ ~loc:ppat_loc ~attrs:ppat_attributes pvb_expr t0))

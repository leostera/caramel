...
module W = struct
  ...
  let to_string x = String.concat ~sep:"" (List.map ~f:(Format.sprintf "%+d") x)
end
...
let parse fragment (conf : Conf.t) ~source =
  ...
  let warnings =
    W.enable 50 :: (if conf.quiet then List.map ~f:W.disable W.in_lexer else [])
  in
  ...
  let t =
    with_warning_filter
      ...
      ~f:(fun () ->
        ...
        let comments =
          List.map ~f:(fun (txt, loc) -> Cmt.create txt loc) (Lexer.comments ())
        in

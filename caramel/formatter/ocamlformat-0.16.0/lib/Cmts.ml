...
let infix_symbol_before src (loc : Location.t) =
  ...
  with
  | Some
      ( ( SEMI | INFIXOP0 _ | INFIXOP1 _ | INFIXOP2 _ | INFIXOP3 _ | INFIXOP4 _
        | COLONCOLON | COLONEQUAL | BARBAR | LESSMINUS | EQUAL | COLON | LETOP _
        | ANDOP _ | DOTDOT | DOTOP _ | AMPERAMPER ),
        _ ) ->
      true
  | _ -> false
  ...
  if not (CmtSet.is_empty cmts) then (
    ...
    if t.debug then
      List.iter cmtl ~f:(fun { Cmt.txt = cmt_txt; loc = cmt_loc } ->
          ...
          in
          let btw_next =
            Option.value_map next ~default:"no next" ~f:(string_between cmt_loc)
          in
  ...
  | curr_loc :: next_locs ->
      ...
      let before_curr =
        ...
        | Some prev_loc ->
            ...
            in
            add_cmts t `After ~prev:prev_loc ~next:curr_loc prev_loc after_prev;
            before_curr
      in
...
let relocate_ext_cmts (t : t) src ((_pre : string Location.loc), pld) ~whole_loc
    =
  match pld with
  | PStr
      [
...
let relocate_wrongfully_attached_cmts t src exp =
  ...
  | Pexp_match (e0, _) ->
      relocate_pattern_matching_cmts t src Parser.MATCH ~whole_loc:exp.pexp_loc
...
let pop_if_debug t loc =
  if t.debug && t.remove then update_remaining t ~f:(fun s -> Set.remove s loc)
...
let line_dist a b = b.Location.loc_start.pos_lnum - a.Location.loc_end.pos_lnum

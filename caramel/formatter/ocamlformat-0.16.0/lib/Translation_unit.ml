...
let print_error ~fmt ~exe ~debug ~quiet ~input_name error =
  ...
  | Invalid_source { exn } -> (
      ...
      match exn with
      | Syntaxerr.Error _ | Lexer.Error _ -> Location.report_exception fmt exn
      | Warning50 l ->
          List.iter l ~f:(fun (l, w) -> print_warning l w);
          Format.fprintf fmt
            "@{<warning>Hint@}: (Warning 50) This file contains a \
             documentation comment (** ... *) that the OCaml compiler does not \
             know how to attach to the AST. OCamlformat does not support these \
             cases. You can find more information at: \
             https://github.com/ocaml-ppx/ocamlformat#overview. If you'd like \
             to disable this check and let ocamlformat make a choice (though \
             it might not be consistent with the ocaml compilers and odoc), \
             you can set the --no-comment-check option.\n\
             %!"
                ...
                | Normalize.Unstable (loc, s) ->
                    Format.fprintf fmt
                      "%!@{<loc>%a@}:@,\
                       ...
                       (e.g. parses as a list or not depending on the margin), \
                       please tighten up this comment in the source or disable \
                       the formatting using the option --no-parse-docstrings.\n\
                       %!"
...
let check_all_locations fmt cmts_t =
  ...
  | l ->
      ...
      Format.fprintf fmt "Warning: Some locations have not been considered\n%!";
...
let format fragment ?output_file ~input_name ~source ~parsed conf opts =
  ...
  let rec print_check ~i ~(conf : Conf.t) t ~source =
      ...
      let contents =
        with_buffer_formatter ~buffer_size:(String.length source)
          ...
          $ with_optional_box_debug ~box_debug
              (Fmt_ast.fmt_fragment fragment ~debug:opts.debug source_t cmts_t
                 conf t.ast))
      in
      (contents, cmts_t)
    in
    ...
    if String.equal source fmted then (
      ...
      (if
       not
         (equal fragment ~ignore_doc_comments:(not conf.comment_check) conf t
            t_new)
      then
       let old_ast = dump_ast ~suffix:".old" (normalize fragment conf t) in
       let new_ast = dump_ast ~suffix:".new" (normalize fragment conf t_new) in
       let args ~suffix =
         [
       ...
       if equal fragment ~ignore_doc_comments:true conf t t_new then
         let docstrings =
           Normalize.moved_docstrings fragment conf t.Parse_with_comments.ast
...
let parse_and_format fragment ?output_file ~input_name ~source conf opts =
  Ocaml_common.Location.input_name := input_name;
  let open Result.Monad_infix in
  parse_result fragment conf opts ~source ~input_name >>= fun parsed ->

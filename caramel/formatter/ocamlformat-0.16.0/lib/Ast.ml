...
module Structure_item = struct
  ...
  let is_simple (itm, c) =
    ...
    | `Compact | `Preserve -> Location.is_single_line itm.pstr_loc c.Conf.margin
    | `Sparse -> (
        match itm.pstr_desc with
...
module Signature_item = struct
  ...
  let is_simple (itm, c) =
    match c.Conf.module_item_spacing with
    | `Compact | `Preserve -> Location.is_single_line itm.psig_loc c.Conf.margin
    | `Sparse -> (
        match itm.psig_desc with
...
let break_between s ~cmts ~has_cmts_before ~has_cmts_after (i1, c1) (i2, c2) =
  ...
  | Sig i1, Sig i2 ->
      Signature_item.break_between s ~cmts ~has_cmts_before ~has_cmts_after
        (i1, c1) (i2, c2)
  | Exp i1, Exp i2 ->
      Exp.break_between ~cmts ~has_cmts_before ~has_cmts_after (i1, c1) (i2, c2)
  | Vb i1, Vb i2 ->
      Vb.break_between ~cmts ~has_cmts_before ~has_cmts_after (i1, c1) (i2, c2)
  | Mty _, Mty _ ->
...
and Requires_sub_terms : sig
  ...
  let check_typ { ctx; ast = typ } =
    ...
    in
    let check_typexn { ptyexn_constructor; _ } = check_ext ptyexn_constructor in
    let check_type { ptype_params; ptype_cstrs; ptype_kind; ptype_manifest; _ }
        =
      List.exists ptype_params ~f:fst_f
      || List.exists ptype_cstrs ~f:(fun (t1, t2, _) -> typ == t1 || typ == t2)
          ...
          | Pcf_method
            ...
            ->
              let rec loop = function
                ...
                | _ -> false
              in
              (match topt with None -> false | Some t -> typ == t) || loop e
    ...
    | Cl { pcl_desc; _ } ->
        assert (
          ...
          | Pcl_structure { pcstr_fields; _ } -> check_pcstr_fields pcstr_fields)
    | Mty ctx ->
        let rec loop m =
          match m with
          | Pmty_with (m, c1N) ->
              List.exists c1N ~f:(function
                | Pwith_type (_, d1) | Pwith_typesubst (_, d1) -> check_type d1
                | _ -> false)
              || loop m.pmty_desc
          | _ -> false
        in
    ...
    let check_class_type l =
      List.exists l ~f:(fun { pci_expr; _ } ->
          let rec loop x =
            x == cty
            ||
            match x.pcty_desc with Pcty_arrow (_, _, x) -> loop x | _ -> false
          in
          loop pci_expr)
    in
    match (ctx : t) with
  ...
  let check_cl { ctx; ast = cl } =
    let check_pcstr_fields pcstr_fields =
      List.exists pcstr_fields ~f:(fun f ->
          match f.pcf_desc with Pcf_inherit (_, x, _) -> x == cl | _ -> false)
    in
    match (ctx : t) with
    | Exp e -> (
        match e.pexp_desc with
  ...
  let check_pat { ctx; ast = pat } =
    ...
    | Exp ctx -> (
        ...
        | Pexp_constant _ | Pexp_constraint _ | Pexp_construct _ | Pexp_field _
        | Pexp_ident _ | Pexp_ifthenelse _ | Pexp_lazy _ | Pexp_letexception _
  ...
  let check_exp { ctx; ast = exp } =
    ...
    | Exp ctx -> (
        ...
        | Pexp_apply (e0, e1N) -> assert (e0 == exp || List.exists e1N ~f:snd_f)
  ...
  let prec_ctx ctx =
    ...
    | { ctx = Exp { pexp_desc; _ }; ast = Exp exp } -> (
        match pexp_desc with
        | Pexp_tuple (e0 :: _) -> Some (Comma, if exp == e0 then Left else Right)
        | Pexp_construct
            ( { txt = Lident "::"; _ },
    ...
    | {
        ctx = Exp _;
        ast = Pld _ | Top | Tli _ | Pat _ | Cl _ | Mty _ | Mod _ | Sig _ | Str _;
      }
    | { ctx = Vb _; ast = _ }
    | { ctx = _; ast = Vb _ }
    | {
    ...
    | {
        ctx =
          ( Pld _ | Top | Tli _ | Typ _ | Cty _ | Pat _ | Mty _ | Mod _ | Sig _
          | Str _ );
        ast =
  ...
  let parenze_pat ({ ctx; ast = pat } as xpat) =
    ...
    | ( ( Pat
            {
              ppat_desc =
                ( Ppat_construct _ | Ppat_exception _ | Ppat_or _ | Ppat_lazy _
    ...
    | ( Exp
          {
            pexp_desc = Pexp_fun _ | Pexp_function _ | Pexp_match _ | Pexp_try _;
            _;
          },
        Ppat_constraint _ )
  ...
  let rec exposed_right_exp =
    ...
    fun cls exp ->
      let exposed_ () =
        ...
        | Pexp_match _ when match cls with Then -> true | _ -> false -> false
        | Pexp_function cases | Pexp_match (_, cases) | Pexp_try (_, cases) ->
  ...
  and exposed_right_cl =
    ...
    fun cls cl ->
      let exposed_ () =
        ...
        | Pcl_fun (_, _, _, e) ->
            (not (parenze_cl (sub_cl ~ctx:(Cl cl) e))) && exposed_right_cl cls e
        | _ -> false
      in
      Cl.mem_cls cls cl || Hashtbl.find_or_add memo (cls, cl) ~default:exposed_
  ...
  and mark_parenzed_inner_nested_match exp =
    let exposed_ () =
      ...
      | Pexp_array _ | Pexp_coerce _ | Pexp_constant _ | Pexp_constraint _
      | Pexp_construct (_, None)
      | Pexp_extension _ | Pexp_field _ | Pexp_for _ | Pexp_ident _ | Pexp_new _
      | Pexp_object _ | Pexp_override _ | Pexp_pack _ | Pexp_poly _
      | Pexp_record _ | Pexp_send _ | Pexp_unreachable
      | Pexp_variant (_, None)
      | Pexp_while _ ->
          false
    in
    Hashtbl.find_or_add marked_parenzed_inner_nested_match exp ~default:exposed_
    |> (ignore : bool -> _)

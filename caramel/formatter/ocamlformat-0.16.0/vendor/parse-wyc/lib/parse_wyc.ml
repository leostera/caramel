...
module State = struct
  type t = { result : Location.t list; stack : Location.t list }

  let result s = s.result

  let empty = { result = []; stack = [] }

  let add s = { s with result = List.hd s.stack :: s.result }

  let add_if cond s = if cond then add s else s

  let push_on_stack loc s = { s with stack = loc :: s.stack }

  let pop_stack s = { s with stack = List.tl s.stack }

  let wrap loc f x s =
    match s.stack with
    | [] -> push_on_stack loc s |> f x |> pop_stack
    | _ -> f x s
end
...
let invalid_locs_fold =
  object
    ...
    method! attribute x s =
      State.add_if (Annot.Attr.is_generated x) s |> super#attribute x

    method! expression e s =
      State.add_if (Annot.Exp.is_generated e) s |> super#expression e

    method! class_expr x s =
      State.add_if (Annot.Class_exp.is_generated x) s |> super#class_expr x

    method! structure_item x s = State.wrap x.pstr_loc super#structure_item x s

    method! signature_item x s = State.wrap x.psig_loc super#signature_item x s
  end
...
let mk_parsable fragment source =
  ...
  | invalid_locs ->
      ...
      let wrapper_opn, wrapper_cls =
        ( Printf.sprintf "[%s {%s|" extension_name delim,
          Printf.sprintf "|%s}]" delim )
      in
      let wrapper_len = String.length wrapper_opn + String.length wrapper_cls in
      let source_len = String.length source in
      let len = source_len + (List.length invalid_locs * wrapper_len) in

...
let to_bytecode i lambda =
  ...
  (fun Lambda.{ code = lambda; required_globals; _ } ->
    ...
    let bytecode = simp_lambda |> Bytegen.compile_implementation i.module_name in
...
let erl_to_cmi ~source_file ~output_prefix =
  ...
  Compile_common.with_info
    ...
    @@ (fun i ->
      ...
      let typedtree = parsetree |> Profile.(record typing) (Typemod.type_implementation i.source_file i.output_prefix i.module_name i.env)
      in
      backend i typedtree;
      )

let compile_one source =
  let (fn, source_file) = match source with
...
let compile { sources; _ } =
  match
    ...
    let (tagged_sources, errs) = sources |> List.map tag_source |> List.partition (function `Unsupported_file _ -> false | _ -> true ) in

    errs
    |> List.iter (function
      | `Unsupported_file_type (file, ext) ->
          Format.fprintf Format.std_formatter "ERROR: Filename %s has unsupported extension %s." file ext;
          exit 1
      | _ -> ()
    );
  ...
  | exception exc -> Format.fprintf Format.std_formatter "ERROR: %s" (Printexc.to_string exc)
  | _ -> ()

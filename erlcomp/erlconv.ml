...
let longident_to_type_name x =
  ...
  | n_name :: mods -> (
      ...
      | "erlang", "process" -> Erlast.(Qualified_name { n_mod; n_name = "pid" })
      | _, _ -> Erlast.(Qualified_name { n_mod; n_name }) )

let to_erl_op t =
  Erlast.(Qualified_name { n_mod = "erlang"; n_name = "'" ^ t ^ "'" })
...
let const_to_literal const =
  ...
  | Const_nativeint nativeint ->
      Erlast.Lit_integer (Nativeint.to_string nativeint)
...
let build_functions :
 ...
 fun ~module_name ~modules typedtree ->
  ...
  let rec build_function fd_name cases ~var_names =
    ...
    in
    Some Erlast.{ fd_name; fd_arity; fd_cases }
  ...
  and build_pattern : type k. k general_pattern -> Erlast.pattern =
   fun pat ->
    ...
    | Tpat_tuple tuples -> Erlast.Pattern_tuple (List.map build_pattern tuples)
    | Tpat_record (fields, _) ->
        Erlast.Pattern_map
          ( fields
          |> List.map (fun (Asttypes.{ txt }, _, pattern) ->
  ...
  and build_expression exp ~var_names =
    ...
    | Texp_ident (_, { txt }, _) -> (
        ...
        if name_in_var_names ~var_names var_name then
          Some (Erlast.Expr_name (Erlast.Var_name var_name))
          ...
          | Erlast.Qualified_name _, false -> Some (Erlast.Expr_name name)
    ...
    | Texp_match (expr, branches, _) ->
        ...
        let branches : Erlast.case_branch list =
          branches
          |> List.map (fun c ->
    ...
    | Texp_ifthenelse (if_cond, if_true, if_false) ->
        ...
        let if_true =
          Erlast.
            {
              cb_pattern = Erlast.Pattern_match (Erlast.Lit_atom "true");
        ...
        let if_false =
          ...
          | Some if_false ->
              [
                Erlast.
                  {
                    cb_pattern = Erlast.Pattern_match (Erlast.Lit_atom "false");
                    cb_expr =
    ...
    | Texp_let (_, vbs, expr) ->
        ...
        *)
        ...
        let var_names =
          collect_var_names Erlast.[ let_binding.lb_lhs ] @ var_names
        in
...
let build_types :
 ...
 fun typedtree signature ->
  ...
  let rec build_type_kind core_type =
    ...
    | Ttyp_var var_name ->
        Some (Erlast.Type_variable (var_name |> varname_of_string))
...
let build_exports :
 ...
 fun ~name:_ typedtree signature ->
  ...
  |> List.filter_map (fun sig_item ->
         match sig_item with
         | Sig_value (_, { val_kind = Val_prim _ }, Exported) -> None
       ...
       (fun acc struct_item ->
         let mbs =
           ...
           |> List.concat_map (fun mb ->
                  ...
                  | Tmod_constraint
                      ( { mod_desc = Tmod_structure typedtree },

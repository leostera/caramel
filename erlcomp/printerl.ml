...
and pp_type_kind prefix ppf typ_kind =
  ...
  | Type_constr { tc_name = Atom_name "unit" } -> Format.fprintf ppf "ok"
  ...
  | Type_record { fields } -> (
      ...
      | Erlast.{ rf_name; rf_type } :: fs -> (
          ...
          | fs ->
              Format.fprintf ppf "\n";
              fs
              |> List.iter (fun Erlast.{ rf_name; rf_type } ->
  ...
  | Expr_case (expr, branches) -> (
      ...
      | Erlast.{ cb_pattern; cb_expr } :: bs ->
          (let prefix = prefix ^ "  " in
           ...
           | bs ->
               bs
               |> List.iter (fun Erlast.{ cb_pattern; cb_expr } ->
                      Format.fprintf ppf ";\n%s" prefix;
                      pp_pattern_match ppf cb_pattern;
                      Format.fprintf ppf " -> ";
                      pp_expression "" ppf cb_expr ~module_));
  ...
  | Expr_map fields -> (
      ...
      | Erlast.{ mf_name; mf_value } :: fs -> (
          ...
          | fs ->
              Format.fprintf ppf "\n";
              fs
              |> List.iter (fun Erlast.{ mf_name; mf_value } ->

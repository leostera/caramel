...
let rec phi_aux acc = function
  | [] -> acc
  | (p, m) :: t -> phi_aux ((p - 1) * pow p (m - 1) * acc) t

let phi_improved n = phi_aux 1 (factors n)

let main _ =
  format "~p\n"
    [
      [("phi_improved 10", phi_improved 10); ("phi_improved 13", phi_improved 13);]
    ]

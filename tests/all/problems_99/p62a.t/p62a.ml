...
let rec at_level_aux level t acc counter =
  match t with
  | Empty -> acc
  | Node (x, l, r) ->
      if counter = level then x :: acc
      else at_level_aux level l (at_level_aux level r acc (counter + 1)) (counter + 1)

let at_level t level = at_level_aux level t [] 1
...
let main _ =
  ...
  in
  format "~p\n" [ [ at_level example 2; at_level example 5 ] ]
